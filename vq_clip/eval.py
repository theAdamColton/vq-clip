...
def zero_shot_eval(
    model: CLIPModel, processor: CLIPProcessor, imagenet_path: str, batch_size=32
):
    was_training = model.training
    model.eval()
    print("Starting zero-shot imagenet.")

    ds = torchvision.datasets.ImageFolder(imagenet_path)

    def collate(x):
        images, labels = list(zip(*x))
        images = processor(images=images, return_tensors="pt")["pixel_values"]
        labels = torch.LongTensor(labels)
        return images, labels

    dl = DataLoader(
        ds, batch_size=batch_size, shuffle=False, collate_fn=collate, num_workers=2
    )
    ...
    with torch.no_grad():
        ...
        for images, target in tqdm(dl, unit_scale=batch_size):
